...
%\abbrevs{EDMF, Eddy Diffusivity Mass Flux; TVD, Total Variation Diminishing; KE, Kinetic Energy; PE, Potential Energy; IE, Internal Energy; RMS; Root Mean Square; } 
...
\begin{abstract}
The representation of sub-grid scale convection is a weak aspect of weather and climate prediction models and the assumption that no net mass is transported by convection in parameterisations is increasingly unrealistic as models enter the grey zone, partially resolving convection. The solution of conditionally averaged equations of motion is proposed in order to avoid this assumption. Separate continuity, temperature and momentum equations are solved for inside and outside convective plumes which interact via mass transfer terms, drag and by a single common pressure. This is not a convection scheme that can be used with an existing dynamical core -- this requires a whole new model.

This paper presents stable numerical methods for solving the conditionally averaged equations of motion including large transfer terms between the environmental and plume fluids. Without transfer terms the two fluids are not sufficiently coupled together and solutions diverge. Two transfer terms are presented which couple the fluids together in order to stabilise the model: diffusion of mass between the fluids (similar to turbulent entrainment) and drag between the fluids. Transfer terms are also proposed to move buoyant convecting air into the plume fluid and vice-versa as would be needed to represent
\textcolor{blue}{initialisation and termination of}
sub-grid-scale convection. These transfers mimic  cloud base mass flux and cloud top detrainment. The transfer terms are limited (clipped in size) and treated implicitly in order to achieve bounded, stable solutions. 

Results are presented of a well resolved rising warm bubble with rising air being transferred to the \textcolor{blue}{plume} fluid. For stability, equations are formulated in advective rather than flux form and solved using bounded finite volume methods. Discretisation choices are made to preserve boundedness and conservation of momentum and energy when mass is transferred between fluids.
...
The problem gets worse when modelling convection
...
\end{enumerate}
Once a convection scheme transports mass as well
as heat, moisture and momentum, all of the same equations of motion
should be solved both inside and outside the convective plumes. \textcolor{blue}{Therefore} 
it no longer makes sense to have a convection paramterisation as a
module
\textcolor{blue}{external}
to a dynamical core -- they should both be solving
...
the vertical direction \citep[eg. ][]{WLW13}, so that they do not
...
uses to control divergence. If \textcolor{blue}{a significant fraction} of a grid box contains
...
of zero net mass flux in each column. The conditionally averaged equations
...
due to convection, exceptions being \citet{KB08,AW13}. \citet{KGB07,KB08}
...
in fact ill-posed \citep{SW84} and are usually regularised by including
...
of the numerical method as they are too big to be
treated explicitly and they will introduce new
\textcolor{blue}{mass, temperature and momentum}
 extrema
\textcolor{blue}{in one fluid}. We use two
...
similar to lateral entrainment and detrainment. 
...
is one if the fluid is defined to be fluid $i$ at
that location and zero otherwise. \textcolor{blue}{Fluid $i=0$ may be for example the stable environment, fluid $i=1$ convective plumes and fluid $i=2$ may be downdrafts.}
A filter (typically volume average)
...
\end{eqnarray}
As with Reynolds or Favre averaging, non-linear
conditionally averaged terms such as $\widetilde{I_{i}\rho\mathbf{u}\theta}$
...
\end{eqnarray}
\textcolor{blue}{where $\mathbf{F}_{\text{SF}}^{\theta_{i}}$ is a vector
and $\mathsf{F}_{\text{SF}}^{\mathbf{u}_{i}}$ is a tensor.}
The sub-filter scale fluxes could be due to turbulent motions within
each fluid. Parameterisations for sub-filter scale
...
pressure is uniform across all fluids. \textcolor{blue}{Pressure differences}
between the plume and the environment will \textcolor{blue}{be parameterised as} form drag which is discussed in section \ref{subsec:drag}.
...
Applying conditional averaging to the dry, adiabatic,
...
to the following conditionally averaged Euler equations for
...
is the heat transfer from fluids $i$ to $j$. 
\textcolor{blue}{Formulation of the transfer terms, $S_{ij}$, $H_{ij}$ and $\mathbf{d}_{ij}$ is needed to create a parameterisation of convection. The mass transfer, $S_{ij}$, will be dependent on the definitions of each fluid and on sub-grid scale variability. Heat transfer and drag between fluids, $H_{ij}$ and $\mathbf{d}_{ij}$, will depend on the surface area between fluids.}
When mass is transferred,
we assume that its mean properties
...
\end{equation}
The temperature and momentum equations can be expressed
...
\end{eqnarray}
where $D_{ij}=\sigma_{j}d_{ij}/\rho_{i}$. Note
...
\textcolor{blue}{In order to represent sub-grid scale convection with the multi-fluid equations we first need to give labels to parts of the fluid with different properties. For example, fluid zero could be the stable environment fluid, fluid one could be the convectively unstable buoyant plumes and fluid two could be downdrafts. For now it doesn't matter precisely how this is done. The next essential stage is to formulate transfer terms, $\mathbf{D}_{ij}$,
$S_{ij}$ and $H_{ij}$.} $S_{ij}$ is particularly
important for moving mass in and out of the fluid related to convection
and will \textcolor{blue}{presumably} be associated with sub-grid scale variability
...
that transfer \textcolor{blue}{fluid based on the properties of the resolved flow, neglecting sub-grid-scale variability.}
The purpose of this is to test the stability, boundedness
and conservation properties of the numerical methods rather than proposing
a useful parameterisation of convection.

We also include transfer terms to stabilise the equations.
...
the fluids. \textcolor{blue}{Sensible and radiative heat transfer between fluids is not considered in this paper and appears not to be needed for stability because the temperature of the fluids does not diverge.}

We will consider one form of drag and three types
...
$S_{bij}$ and $S_{hij}$ are forms of cloud base mass flux. \textcolor{blue}{The diffusion of $\sigma$, $S_{dij}$, mixes the fluids in regions where the gradient of $\sigma$ varies rapidly.} $S_{dij}$ \textcolor{blue}{could be}
similar to lateral entrainment and is used to regularise the ill-posed
...
\textcolor{blue}{Pressure differences between the fluid are parameterized as form drag.}
We use a model for the drag on a rising bubble based on \citet{RLD+11} \textcolor{blue}{which is similar to the form used by \citet{SW69} for modelling buoyant plumes and the form recommended by \citet{RC15} for deep convection}.
...
\end{equation}
where $C_{D}$ is a drag coefficient, $r_{c}$ is a radius
and $\rho_{ij}=(\sigma_{i}\rho_{i}+\sigma_{j}\rho_{j})/(\sigma_{i}+\sigma_{j})$. 
\textcolor{blue}{The radius, $r_c$, needs to be the same for both fluids and could be, for example, the plume radius or a length scale for the region of fluid with lower volume fraction.}
\textcolor{blue}{Initial testing without any form of stabilisation showed that the velocity of the vanishing fluid diverges which is clearly unrealistic -- a tiny bubble of a different fluid would move with the main fluid. This can be achieved by making $r_{c}$ small}
as $\sigma_{i}$ becomes small in any fluid which increases the drag between fluids. If we assume a maximum
...
\end{equation}
For a more realistic scheme, future work could draw
on existing parameterisations of cloud radius.
\textcolor{blue}{From geometric arguments, it is reasonable to expect $r_c$ to be small when $\sigma$ is small but stability restrictions on $r_c$ are not known.}

\subsubsection{Diffusion of $\sigma$\label{subsec:diffuseSigma}}

A diffusive mass transfer term is created to smooth
out high curvature or oscillations in $\sigma$. The form used ensures
that total mass is not diffused and that 
\textcolor{blue}{mass transfer terms are never negative}:
\begin{equation}
...
This transfer term has only been derived considering
...
mass flux rather than to regularise the equations: 
\textcolor{blue}{transfer from fluid $i=0$ to fluid $i=1$ could introduce new extrema of $\sigma_i$, $\theta_1$ and $\mathbf{u}_1$.}
Air will rise if buoyancy
...
\textcolor{blue}{These transfer terms do not guarantee that fluid 0 is stable or that fluid 1 is buoyant but that is the ultimate aim.}

\subsubsection{Transfers Based on Horizontal Divergence and Vertical Velocity\label{subsec:hdiv_w_Transfer}}

This transfer term has also been derived considering
...
Solving the conditionally averaged equations needs
an entire atmospheric model rather than just the convection parameterisation.
Once the sub-grid-scale convection interacts with the \textcolor{blue}{grid-scale} continuity equation,
convection can no longer be an isolated parameterisation --
...
Most of the spatial discretisation is not specific
...
Specific to the conditionally averaged equations,
it is important to use bounded advection of $\sigma_{i}$. A 
\textcolor{blue}{total variation diminishing}
(TVD) advection
...
It is crucial to get the time-stepping right for
...
treated implicitly
\textcolor{blue}{for any value of $\sigma_{i}$}.

The terms of the Euler equations involved in acoustic waves are solved
using second-order Crank-Nicolson time-stepping. Other
...
\textcolor{blue}{plume and environment} momentum and continuity equations in the implicit
solution of the pressure equation.

Non-linear terms and advection are treated explicitly
using second-order Runge-Kutta time-stepping for convenience \citep{WLW13}.
An inner loop solves the pressure equation
\textcolor{blue}{for $\pi$ and $\mathbf{u}_i$}
twice with explicit terms
...
\textcolor{blue}{for $\sigma_i \rho_i$ and $\theta_i$}.
The outer iterations are indexed with $k=1,2$.
...
All transfer terms are solved using operator split
...
(the source terms of
\textcolor{blue}{the temperature equation (\ref{eq:condThetaAdv}) momentum equation (\ref{eq:condMomAdv})}
with $\sigma_{i}\rho_{i}$ in the denominator which
can tend to infinity as $\sigma_{i}\rightarrow0$).

\subsubsection{Initialisation \label{subsec:initialisation}}

The prognostic variables are $U_{i}$, $\sigma_{i}\rho_{i}$,
...
$\pi$. This system is overspecified because $\pi$ can be calculated
for all of the $\sigma_{i}\rho_{i}$ and $\theta_{i}$ using the
equation of state. To avoid over specification, only $\mathbf{u}_{i}$,
$\sigma_{i}$, $\theta_{i}$ and $\pi$ are read in at initialisation
and each $\rho_{i}$ is calculated from the equation
...
\textcolor{blue}{(ie $\theta$ for fluid zero)}.

In order to start each time-step, $(\sigma_{i}\rho_{i})^{k=0}$
...
The first equations to be solved in the outer loop
are the continuity equations for each $\sigma_{i}\rho_{i}$ (\ref{eq:condCont}). When
transfer terms are included (which lead to terms with $\sigma_{i}\rho_{i}$ in the denominator
\textcolor{blue}{of the source terms of the temperature (\ref{eq:condThetaAdv}) and momentum (\ref{eq:condMomAdv}) equations})
it is essential to keep $\sigma_{i}\rho_{i}$
...
a van-Leer limiter (section \ref{subsec:vanLeerContinuity}) to
calculate $\sigma_{i}$ inside the divergence for iterations $k=1,2$:
\begin{equation}
...
making the time-stepping second-order accurate.
\textcolor{blue}{$\rho_i$ inside the divergence is calculated using linear interpolation.}
\textcolor{blue}{$(\sigma_{i}\rho_{i})^{k}$ for all values of $k$ and the intermediate values $(\sigma_{i}\rho_{i})^{\prime}$ share the same memory as $(\sigma_{i}\rho_{i})^{n+1}$.}

Only $\sigma_{i}^{n}$ appears on the right hand
...
Next the mass transfer terms are calculated using
...
After the continuity equation, the $\theta_{i}$ equation (\ref{eq:condThetaAdv}) is solved
using operator splitting; first advecting $\theta_{i}$ then applying
the mass transfer terms to the advected $\theta_{i}$. 
...
\textcolor{blue}{The mass transfer
terms are applied implicitly because they can be very large due to
the presence of $\sigma_{i}\rho_{i}$ in the denominator.}
...
Calculation of $\theta_{i}^{k}$ from eqns (\ref{eq:theta0Transfer})
...
in closure assumptions, such as the approximation of the drag (\ref{eq:dragBubble}).
...
Here we describe how a standard semi-implicit algorithm
is adapted to solve the momentum (\ref{eq:condMomAdv})  and continuity (\ref{eq:condCont}) equations from all
...
equation. This is done twice per outer loop, in an
...
\end{multline}
where $V$ is the cell volume. There are no transfer terms in this equation
because
\textcolor{blue}{they cancel when}
summed over fluids. This is a Helmholtz equation
...
\subsubsection{Updates for Consistency \label{subsec:consistency}}

So far, each time-step, we have updated each $\sigma_{i}\rho_{i}$
...
equations and the equation of state. This is overspecified so to
...
Each item below is carried out for all cells before
moving on to the next item. This is crucial for the semi-implicit
formulation.
\begin{enumerate}
\item We start by initialising all iterated variables at
...
\item Calculate $\mathbf{u}_{i}^{\ell}$ from $U_{i}^{\ell}$ using appendix eqn
(\ref{eq:recontructU}).
\end{enumerate}
\end{enumerate}
\item For consistency, update $\Psi$ and $\pi$ at the end of each time-step
from the equation of state (section \ref{subsec:consistency}).
\end{enumerate}
...
fluid divided into two with identical initial conditions in
...
each fluid are identical. The full domain extends
...
advection of velocity (linear-upwind) and potential
temperature (van-Leer). A simulation is also run
...
simulation loses energy \textcolor{blue}{more} slowly and the energy loss is no longer
...
The total energy for the simulations with two identical fluids
...
lines show negative changes. 
\textcolor{blue}{The x-axis is on a square root scale to emphasise the changes that occur early in the simulations.}
\label{fig:energy1part}}
\end{figure*}
...
by black vectors and $\mathbf{u}_{1}$ by red vectors. The
...
raises the pressure above the bubble and forces fluid zero downwards.
...
simulation could be maintained for a little longer
by using a smaller time-step or by treating advection implicitly but
the PDEs that we are solving are unstable \citep{SW84} so numerics
cannot be used to maintain stability indefinitely.

\subsubsection{\textcolor{blue}{Coupling the two fluids with drag or diffusion}}
...
Next the fluids are coupled by adding diffusion between
...
the stability limit. Once mass is transferred, the
...
with the mass so the temperature in fluid 1 no longer
remains 300\,K and the temperature in fluid 0 reduces.
...
leads to more rapid energy loss than any of the other effective stabilisation
methods. A high drag coefficient ($C_{D}=10^{6}$) is also used to
check the stability of the implicit treatment of drag. 
\textcolor{blue}{It is not immediately obvious why the high drag coefficient would lead to less energy loss when we usually think of drag as removing kinetic energy. With high drag between the fluids, kinetic energy is more evenly spread between the fluids and lower velocities and lower shear could lead to less kinetic energy dissipation by the dissipative advection.}
...
should be conserved. In order to maintain stability,
...
figure \ref{fig:massTransfer} using diffusivities
...
warm fluid is completely transferred to fluid one whereas using $K_{\theta}=10^{5}\ \text{m}^{2}\text{s}^{-1}$,
...
the RMS errors are low but not zero. These RMS errors
...
conditions is introducing numerical error although
...
All simulations use $K_{\sigma}=100\ \text{m}^{2}\text{s}^{-1}$
and drag with coefficient of $C_{D}=1/2$, $r_{\min}=1\ \text{m}$
...
lines show negative changes (all changes are negative).
\textcolor{blue}{The x-axis is on a square root scale to emphasise the changes that occur early in the simulations.}
\label{fig:massTransferEnergy}}
\end{figure}
...
RMS differences of $1.9\times10^{-7}$ which again
...
air itself.
\textcolor{blue}{The warm air actually expands horizontally and so is not transferred.}
...
\subsubsection{Transfers based on buoyancy perturbations, horizontal
divergence and vertical velocity}

Finally we use both types of mass transfer in the
...
transferred based on both processes separately but this is not the
...
\begin{enumerate}
\item The conditionally averaged equations with a single pressure are ill-posed so transfer terms are formulated to represent drag and 
\textcolor{blue}{mass transfer}
between fluids to ensure that the different fluids do not diverge. 
...
fraction, $\sigma_{i}$, and removes time-step restrictions
based on the speed of sound. This aspect of the solution entails
substituting
\textcolor{blue}{momentum equations of both (or all) fluids}
into the continuity
...
The results presented in this paper use explicit
...
Results are presented of a well resolved rising warm bubble simulated
...
mass between the fluids or by adding drag between the fluids. Transfer
terms are also included to move the warm rising air from the stable
fluid to the buoyant fluid.
\textcolor{blue}{The model loses energy slowly due mainly to dissipative advection. The transfer terms are implemented for stability and so that internal energy is conserved.}

This paper has not answered the question of how much
...
stabilisation will be the subject of future work.

The formulation of transfer terms in order to represent sub-grid convection
is also the subject of future work. These transfer terms should depend
on sub-grid variability of the primitive variables in each fluid.
For representing unresolved flow, parameterisations
...
This appendix defines the spatial discretisation
...
this study are all fully structured.

\subsection{Reconstruction of velocity fields at cell centres and faces from
face normals}

The model uses a C-grid so normal components of the
...
This reconstruction is first-order accurate on arbitrary
...
This section is again concerned with the discretisation
...
interpolated from cell centres to faces using the OpenFOAM (second-order)
...
of C-grid advection straightforward in OpenFOAM. The
...
momentum could be created or destroyed when mass
is transferred. 

\subsection{Advection of $\sigma_{i}\rho_{i}$\label{subsec:vanLeerContinuity}}

Here we consider the discretisation of the advection
term of the continuity equation (\ref{eq:condCont}). The mass transfer
terms that appear in the $\theta$ and momentum equations
...
Here we consider the advection term in the temperature
